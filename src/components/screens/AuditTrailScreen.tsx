...
export const AuditTrailScreen: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-8 space-y-10 animate-fade-in">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800">
        <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
            ...
            return (
              <div
                ...
                className={`p-4 rounded-xl border transition-all cursor-pointer ${
                  isSelected
                    ? 'border-emerald-500 bg-slate-900 text-white shadow-xs dark:border-emerald-400 dark:bg-slate-800'
                    : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-[#1A1F2E] hover:border-slate-300 dark:hover:border-slate-700 text-slate-800 dark:text-slate-200'
                }`}
              >
                <div className="flex items-center justify-between text-xs">
                  <span className={`font-mono font-bold ${isSelected ? 'text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`}>
                    {event.timestamp}
                  </span>
                  <span className={`font-mono text-[10px] uppercase tracking-wider ${isSelected ? 'text-slate-400' : 'text-slate-400 dark:text-slate-500'}`}>
                    {event.category}
                  </span>
                </div>
                ...
                <div className="mt-1.5 flex items-center justify-between">
                  ...
                  <ChevronRight size={13} className={isSelected ? 'text-emerald-400' : 'text-slate-300 dark:text-slate-600'} />
                </div>
              </div>
            );
          })}
        ...
        <div className="md:col-span-6 space-y-4">
          <div className="bg-white dark:bg-[#1A1F2E] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-xs space-y-5 sticky top-6">
            ...
            <div className="space-y-4 text-xs">
              <div>
                <span className="text-slate-400 dark:text-slate-500 font-mono text-[11px] uppercase tracking-wider block mb-1">
                  Event
                </span>
                <div className="font-bold text-slate-900 dark:text-white text-sm">{activeEvent.title}</div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 pt-0.5">
                    {activeEvent.previousStatus ? (
                      ...
                      <span className="text-slate-400 dark:text-slate-500 font-mono">UNSET</span>
                    )}
              ...
              <div>
                ...
                <div className="p-2.5 rounded bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 font-mono text-slate-800 dark:text-slate-200">
              ...
              <div>
                ...
                <div className="p-2.5 rounded bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 leading-relaxed font-sans">
